...
numOfPlayers = 10
matchmakingConfigName="abp-demo-config-latency"

clients = [] # List of client
threads = []

def update_player_latency() :
  clients[0]['LatencyInMS'] = {'custom-ap-northeast-2' : 400, 'custom-us-east-1': 70 }
  clients[1]['LatencyInMS'] = {'custom-ap-northeast-2' : 150, 'custom-us-east-1' : 180 }
  clients[2]['LatencyInMS'] = {'custom-ap-northeast-2': 30, 'custom-us-east-1': 500 }
  clients[3]['LatencyInMS'] = {'custom-ap-northeast-2': 80, 'custom-us-east-1': 4000 }
  clients[4]['LatencyInMS'] = {'custom-ap-northeast-2': 360, 'custom-us-east-1': 140 }
  clients[5]['LatencyInMS'] = {'custom-ap-northeast-2': 450, 'custom-us-east-1': 430 }
  clients[6]['LatencyInMS'] = {'custom-ap-northeast-2': 500, 'custom-us-east-1': 111 }
  clients[7]['LatencyInMS'] = {'custom-ap-northeast-2': 190, 'custom-us-east-1': 219 }
  clients[8]['LatencyInMS'] = {'custom-ap-northeast-2': 350, 'custom-us-east-1': 128 }
  clients[9]['LatencyInMS'] = {'custom-ap-northeast-2': 250, 'custom-us-east-1': 520 }
...
# sock.setsockopt(socket.IPPROTO_TCP, socket.TCP_KEEPALIVE, 1) # TCP_KEEPALIVE not supported on MAC
  ...
  while t < 30:
    ...
    if t == 30 : # send ClientPing every 30 sec.
      ...
      sock.send(var);
      print('[player', i,'] ClientPing sent to server')
      t = 0
    time.sleep(1)
...
for i in range (numOfPlayers):
  match_response = gl_client.start_matchmaking(
    ...
    Players = [ 
      { 
        'PlayerId' : str(clients[i]['playerId']), 
        'PlayerAttributes' : clients[i]['playerAttr'],
        #'LatencyInMs': {'custom-ap-northeast-2':300, 'custom-us-west-2': 400 } 
        'LatencyInMs': clients[i]['LatencyInMS']
      } 
    ]
  )
  ...
  for i in range (numOfPlayers):
    if clients[i]['TicketStatus'] != 'COMPLETED': 
      ...
      if clients[i]['TicketStatus'] == 'COMPLETED':
        ...
        result = ddb_table.get_item( Key= {'DestinationSocketAddress.IpAddress' : clients[i]['ipaddr'], 'DestinationSocketAddress.Port' : clients[i]['port'] })
        if 'Item' in result : 
          clients[i]['ipaddr']  = "15.197.237.218"
          clients[i]['port']    = int(result['Item']['AcceleratorPort'])
